...
use crate::update::{check_for_updates, perform_self_update};
...
struct Args {
    ...
    /// Show detailed execution plan with levels and dependencies
    #[arg(long)]
    show_plan: bool,

    /// Prints verbose output including recipe execution details
    #[arg(short, long)]
    verbose: bool,
}
...
async fn handle_list_templates(args: &Args) -> anyhow::Result<()> {
    let project =
        load_project_with_feedback(&args.path, &[], args.force_version_override, args.verbose)
            .await?;

    if project.template_registry.is_empty() {
        println!(
            "No templates found in {}",
            project.get_project_templates_path().display()
        );
        ...
        if !template.parameters.is_empty() {
            ...
            for (param_name, param_def) in &template.parameters {
                ...
                let default = if let Some(default) = &param_def.default {
                    format!(
                        " [default: {}]",
                        serde_yaml::to_string(default).unwrap_or_default().trim()
                    )
                } else {
                    String::new()
                };
                println!(
                    "     • {}: {:?}{}{}",
                    param_name, param_def.parameter_type, required, default
                );
...
async fn handle_validate_templates(args: &Args) -> anyhow::Result<()> {
    ...
    for (name, template) in &project.template_registry {
        ...
        for (param_name, param_def) in &template.parameters {
            if param_def.required && param_def.default.is_some() {
                errors.push(format!(
                    "Parameter '{param_name}' is marked as required but has a default value"
                ));
            }
            ...
            if let Some(pattern) = &param_def.pattern {
                if regex::Regex::new(pattern).is_err() {
                    errors.push(format!(
                        "Parameter '{param_name}' has invalid regex pattern: {pattern}"
                    ));
                }
            }
        }
    ...
    if validation_errors == 0 {
        println!(
            "{} All {} template(s) are valid!",
            console::style("✓").green(),
            project.template_registry.len()
        );
    } else {
        println!(
            "{} {} template(s) have validation errors",
            console::style("✗").red(),
            validation_errors
        );
    }
...
async fn handle_render(args: &Args) -> anyhow::Result<()> {
    ...
    // Display execution plan based on flags (same logic as in baker)
    if args.show_plan {
...
async fn run_bake(args: Args) -> anyhow::Result<()> {
    let mut project = load_project_with_feedback(
        &args.path,
        &args.var,
        args.force_version_override,
        args.verbose,
    ...
    let execution_plan = arc_project.get_recipes_for_execution(recipe_filter, args.regex)?;
    ...
    match baker::bake(arc_project.clone(), cache, execution_plan, args.show_plan).await {
        Ok(_) => Ok(()),
        Err(err) => Err(err),
    }
}

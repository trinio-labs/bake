...
pub fn parse_template(
    ...
    match handlebars.render("template", &json!({ "env": env_values, "var": variables })) {
...
pub fn parse_variable_list(
    ...
    variables
        ...
        .try_fold(IndexMap::new(), |mut acc, (k, v)| {
            let parsed_var = parse_template(v, environment, &acc)?;
...
mod test {
    ...
    fn test_parse_template() {
        ...
        let result = parse_template("{{var.foo}}", environment.as_slice(), &variables).unwrap();
    ...
    fn test_parse_variable_list() {
        ...
        let result = parse_variable_list(environment.as_slice(), &variables).unwrap();
